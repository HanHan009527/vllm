...
from vllm.distributed.kv_transfer.kv_connector.v1.mooncake.mooncake_connector import (
    ...
    _filter_remote_block_ids_for_pcp_rank,
...
@pytest.mark.parametrize(
    ("pcp_rank", "expected_counts"),
    [
        (0, [[256, 0]]),
        (1, [[44]]),
    ],
)
def test_remote_block_token_counts_follow_prefill_pcp_rank(
    pcp_rank: int,
    expected_counts: list[list[int]],
):
    """Partial final blocks must be clipped after PCP ownership filtering."""

    assert _remote_block_token_counts_for_pcp_rank(
        remote_block_ids_per_group=[[20, 21, 22]],
        num_external_tokens=300,
        pcp_size=2,
        pcp_rank=pcp_rank,
        block_size=256,
...
def test_remote_prefill_pcp_filter_uses_external_start_token():
    """Remote-prefill blocks keep their global CP ownership after local hits."""

    assert _filter_remote_block_ids_for_pcp_rank(
        remote_block_ids_per_group=[[20]],
        pcp_size=2,
        pcp_rank=0,
        block_size=256,
        cp_kv_cache_interleave_size=256,
...
async def test_build_transfer_params_keeps_partial_remote_prefill_block_for_pcp(
    ...
    transfer_id = "xfer-pcp-offset"
    send_meta = SendBlockMeta(
        p_req_id=f"p-req-pcp-offset-{pcp_rank}",
        transfer_id=transfer_id,
        local_block_ids=[local_blocks],
        ready=asyncio.Event(),
    )
    xfer_meta = MooncakeXferMetadata(
        ...
        remote_tp_size=1,
        remote_tp_rank=0,
        req_blocks={"d-req-pcp-offset": (transfer_id, [[20]])},
        req_num_tokens={"d-req-pcp-offset": 55},
        req_start_tokens={"d-req-pcp-offset": 256},
        kv_caches_base_addr=[remote_region.base_addr],
        block_lens=[remote_region.block_len],
        registered_layer_names=[remote_region.layer_name],
        registered_layer_indices=[remote_region.layer_index],
    ...
    (
        ...
        ready_reqs=[("d-req-pcp-offset", send_meta)],
        agent_meta=xfer_meta,
        local_regions=[local_region],
        remote_regions=[remote_region],
    )
...
async def test_build_transfer_params_transfers_full_partial_remote_prefill_block():
    ...
    transfer_id = "xfer-partial-pcp"
    send_meta = SendBlockMeta(
        p_req_id="p-req-partial",
        transfer_id=transfer_id,
        local_block_ids=[[10, 11]],
        ready=asyncio.Event(),
    )
    xfer_meta = MooncakeXferMetadata(
        ...
        remote_tp_size=1,
        remote_tp_rank=0,
        req_blocks={"d-req-partial": (transfer_id, [[20, 21, 22]])},
        req_num_tokens={"d-req-partial": 556},
        kv_caches_base_addr=[remote_region.base_addr],
        block_lens=[remote_region.block_len],
        registered_layer_names=[remote_region.layer_name],
        registered_layer_indices=[remote_region.layer_index],
    ...
    (
        ...
        ready_reqs=[("d-req-partial", send_meta)],
        agent_meta=xfer_meta,
        local_regions=[local_region],
        remote_regions=[remote_region],
    )
    ...
    assert err_msg is None
    assert src_ptrs == [
        local_region.base_addr + 10 * block_len,
        local_region.base_addr + 11 * block_len,
    ]
    assert dst_ptrs == [
        remote_region.base_addr + 20 * block_len,
        remote_region.base_addr + 22 * block_len,
    ]
    assert lengths == [
        block_len,
        block_len,
...
def test_register_kv_caches_preserves_dsv4_shared_region_group_aliases():
    ...
    assert worker.registered_group_indices == [[0, 1, 2]]
