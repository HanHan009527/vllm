...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        if num_decodes > 0:
            ...
            # DeepGEMM schedule metadata is only needed when its paged MQA
            # logits kernel is available. The sparse indexer has an FP8 torch
            # fallback that ignores this backend-specific buffer.
            if (
                current_platform.is_cuda()
                and is_deep_gemm_paged_mqa_logits_supported()
            ):
